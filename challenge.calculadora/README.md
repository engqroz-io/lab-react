...
> *A calculator application built with React, using styled-components for styling and CSS Grid for responsive layout.*
...
*This application is a fully functional calculator that demonstrates fundamental React concepts, including componentization, state management, and props passing for component customization.*
...
*The application was built using React's **componentization** paradigm, which allows dividing the interface into reusable and independent parts.*
...
*The `Button` component is the main example of reuse and customization via **props** in React. A single component is capable of rendering all calculator buttons with different appearances and behaviors.*
...
*The **spread operator** `{...props}` allows any additional props (like `gridColumn` and `gridRow`) to be automatically passed to the styled-component, which then applies conditional styling:*
...
1. **Componentização**: Divisão da UI em componentes reutilizáveis
   *Componentization: Dividing UI into reusable components*

2. **Estado com Hooks**: Uso de `useState` para gerenciar dados
   *State with Hooks: Using `useState` to manage data*
...
4. **Event Handling**: Tratamento de cliques e interações do usuário
   *Event Handling: Handling user clicks and interactions*
